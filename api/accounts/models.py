...
def create_groups(sender, **kwargs):
    ...
    if sender.name == "accounts":
        ...
        admin_group, _ = Group.objects.get_or_create(name=Account.Role.ADMIN)
        user_group, _ = Group.objects.get_or_create(name=Account.Role.REGULAR)
...
def add_to_admin_or_user_group(sender, instance, created, **kwargs):
    ...
    """
    if created and instance.role == Account.Role.ADMIN:
        admin_group, _ = Group.objects.get_or_create(name=Account.Role.ADMIN)
        if not admin_group.user_set.filter(pk=instance.pk).exists():
            instance.groups.add(admin_group)
    elif created and instance.role == Account.Role.REGULAR:
        user_group, _ = Group.objects.get_or_create(name=Account.Role.REGULAR)
        if not user_group.user_set.filter(pk=instance.pk).exists():
            instance.groups.add(user_group)


class AdminAccountManager(UserManager):
    """
...
class AdminAccount(Account):
    ...
    users = AdminAccountManager()
    base_role = Account.Role.ADMIN
...
@receiver(post_save, sender=AdminAccount)
...
class RegularAccountManager(UserManager):
    """
    Custom manager for RegularAccount to return only regular users.
    """

    def get_queryset(self):
        return super().get_queryset().filter(role=Account.Role.REGULAR)
...
class RegularAccount(Account):
    ...
    users = RegularAccountManager()
    base_role = Account.Role.REGULAR
...
@receiver(post_save, sender=RegularAccount)
def add_to_user_group(sender, instance, created, **kwargs):
    ...
    """
    if created and instance.role == Account.Role.REGULAR:
        user_group, _ = Group.objects.get_or_create(name=Account.Role.REGULAR)
        if not user_group.user_set.filter(pk=instance.pk).exists():
            instance.groups.add(user_group)

...
import { SocialMediaLinks, ThemeToggler } from "../../widgets/base";

export default function FooterBottombarSection({
  themeToggler,
  component,
}: FooterBottombarProps) {
  const { fullName, textPrimary, textColorHover } = useBase();
  ...
  const Footer = () => {
    ...
    const FooterNav = ({ links }: FooterNavProps) => {
      ...
      return (
        <div
          className={`${coreStyles.footer_nav} sm:basis-1/2 flex flex-col items-center`}
        >
          <div>
            {/* Display heading if it exists */}
            {headingLink && (
              <h4 className={`${coreStyles.footer_h4} font-semibold`}>
                {headingLink.text}
              </h4>
            )}
            <nav>
              <ul>
                {otherLinks?.map((link: NavLink, index: number) => (
                  <li key={index} className={coreStyles.footerlinks_li}>
                    <i
    ...
    return (
      <footer
        id="footer"
        className={`${coreStyles.footer} relative border-t border-t-color-tertiary dark:border-t-color-tertiary-reverse`}
      >
        <div
          id="footer-links"
          className={`${coreStyles.footer_main}  bg-body-secondary dark:bg-body-secondary-reverse`}
        >
          <div className="container mx-auto">
            <div className="flex flex-col lg:flex-row gap-y-10 lg:gap-y-0 lg:gap-x-10">
              ...
              <div className="lg:basis-1/3 flex flex-col items-center lg:items-start">
                <h3 className={`${coreStyles.footer_h3}`}>

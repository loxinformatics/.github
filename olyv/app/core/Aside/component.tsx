...
export default function AsideSection() {
  ...
  const { setAsideExists, toggleAside, createNavLinks, navLinksMap, Nav } =
    useCore();
  ...
  const HeadingNavLink = ({ link, index }: any) => {
    return (
      <li
        key={`heading-${index}`}
        className={`${styles.nav_heading} text-color-secondary dark:text-color-secondary-reverse`}
      >
        {link.text}
      </li>
    );
  ...
  return (
    <aside
      id="aside"
      className={`
        ${styles.aside} z-40 w-[300px] p-[20px]

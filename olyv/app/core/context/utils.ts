import { apiURL } from "../../base/context/utils";
import asideToggleStyles from "./styles.module.css";
...
export const createNavDropdown = (children: NavLink[]): NavLink[] => {
  return children.filter((child) => {
    ...
    const invalidTypes: NavLinkType[] = ["login", "logout", "login/logout", "heading", "dropdown"];
...
export const toggleAside = (action: ToggleAsideAction = "toggle") => {
  ...
  switch (action) {
    case "closeOnMobile":
      ...
      if (mediaQuery.matches && document.body.classList.contains(asideToggleStyles.toggle_aside)) {
        document.body.classList.remove(asideToggleStyles.toggle_aside);
      }
      break;

    case "toggle":
    default:
      document.body.classList.toggle(asideToggleStyles.toggle_aside);
      break;
  }
};

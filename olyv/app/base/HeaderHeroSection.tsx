...
function Header({
  ...
  return (
    <header
      ...
      className={`${coreStyles.header} ${otherStyles}`}
    >
        ...
        <div className="w-1/4 flex justify-end items-center">
          {
            ...
            <Btn
              href={heroButtonHref}
              className={`${coreStyles.redirectButton} ${
                showForwardButton && coreStyles.visible
              }`}
            >
              {heroButtonText}
            </Btn>
...
function HeaderNav({ linkColor }: HeaderNavProps) {
  ...
  return (
    <Nav
      className={`text-sm tracking-wider ${coreStyles.header_nav}`}
      links={navLinks}
      layout="header"
...
function ModalNav({ toggleColor }: ModalNavProps) {
  ...
  return (
    !asideExists && (
      ...
      >
        <Nav
          className={coreStyles.modal_nav}
          links={navLinks}
          layout="modal"
          renderLink={renderLink}
        />
...
function Hero({
  ...
  const heroImage = hero_image || "/app/hero.png";
  const heroButtonText = hero_button_text || "Get Started";
  const heroButtonHref = hero_button_href || "";
  ...
  return (
    <Section
      ref={heroRef}
      id={`hero_${sectionId}`}
      className={`text-white w-full ${coreStyles.hero}`}
    ...
    >
      <div className="fixed inset-0">
        <Image
          ...
          priority
          quality={90}
          className={coreStyles.hero_image}
        />
        <div className="absolute inset-0 bg-black/60" />
      </div>
      <div className={`${coreStyles.hero_container} `} data-aos="fade">
        {heroSubHeading && (
          <Heading
            variant="h2"
            className="

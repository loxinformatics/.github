...
function Header({
  ...
  const border = headerBorder ?? true;
  ...
  return (
    <header
      ...
      className={`${styles.header} ${otherStyles}`}
    >
      <div className="flex items-center flex-wrap">
        <div className="w-1/4 flex justify-start items-center">
          <Logo logoVersion={logo_version} textColor={color} />
          <div className="ml-3" />
          {navigation_items && navigation_items.length > 0 && (
            <Navmodal toggleColor={color} navigation_items={navigation_items} />
          )}
          <SidebarToggler />
        </div>
        ...
        <div className="w-1/2 flex justify-center items-center">
          {navigation_items && navigation_items.length > 0 && (
            <Navbar linkColor={color} navigation_items={navigation_items} />
          )}
        </div>
...
function Hero({
  ...
  return (
    <Section
      ref={heroRef}
      id={`hero_${sectionId}`}
      className={`text-white w-full ${styles.hero}`}
    ...
    >
      ...
      <div className={`${styles.hero_container} `} data-aos="fade">
        {heroSubHeading && (
          <Heading
            variant="h2"
            className="
